...
def line_key(value):
    """Converte códigos como 001C, A626 e 026CE.TU para somente números."""
    match = re.search(r"\d+", str(value or ""))
    return match.group(0).zfill(3) if match else ""


def company_key(value):
    """Compara nomes de empresa sem diferenciar acento, caixa ou espaço."""
    text = unicodedata.normalize("NFKD", str(value or ""))
    return "".join(char for char in text if not unicodedata.combining(char)).casefold().replace(" ", "")


def company_name(value):
    """Aplica a grafia oficial das empresas no painel e nas comparações."""
    raw = str(value or "").strip()
    return COMPANIES.get(company_key(raw), raw)
...
def calculate_programmed(ws, start_date, end_date):
    ...
    for row in ws.iter_rows(min_row=3, max_col=CALENDARIO_FIM, values_only=True):
        company = company_name(row[COL_EMPRESA - 1] if len(row) >= COL_EMPRESA else "")
...
def find_non_operated_sheet(workbook):
    required = {"data", "empresa", "km nominal", "não realizadas"}
    for ws in workbook.worksheets:
        header = next(ws.iter_rows(min_row=1, max_row=1, values_only=True), ())
        names = {str(value or "").strip().casefold() for value in header}
        if required.issubset(names) and ({"linha", "linha a"} & names):
            return ws
    raise ValueError("A planilha de KM Não Realizada precisa ter as colunas Data, Empresa, Linha ou Linha A, Não Realizadas e KM NOMINAL.")


def non_operated_columns(header):
    names = [str(value or "").strip().casefold() for value in header]
    ...
    def first(*options):
        ...
        raise ValueError(f"Coluna obrigatória não encontrada: {' / '.join(options)}")

    # Junho possui colunas repetidas. A primeira Empresa e a coluna Linha A
    # são as que trazem os identificadores operacionais corretos.
    return {"data": first("data"), "empresa": first("empresa"), "linha": first("linha a", "linha"), "não realizadas": first("não realizadas"), "km nominal": first("km nominal")}
...
def read_non_operated_km(source, start_date, end_date):
    ...
    try:
        ...
        header = next(ws.iter_rows(min_row=1, max_row=1, values_only=True), ())
        columns = non_operated_columns(header)
        max_col = max(columns[name] for name in ("data", "empresa", "linha", "não realizadas", "km nominal")) + 1
        by_line, trips_by_line, count = defaultdict(float), defaultdict(float), 0
        for row in ws.iter_rows(min_row=2, max_col=max_col, values_only=True):
            ...
            if not current or not start_date <= current <= end_date:
                continue
            company = company_name(row[columns["empresa"]] if len(row) > columns["empresa"] else "")
            ...
            trips_by_line[(company, line)] += number(row[columns["não realizadas"]] if len(row) > columns["não realizadas"] else 0)
            count += 1
        if not count:
            raise ValueError("Não foram encontrados registros de KM Não Realizada no período escolhido.")
        return by_line, trips_by_line
    finally:
        workbook.close()


def line_efficiency(planned_by_line, non_operated_by_line, non_operated_trips=None):
    non_operated_trips = non_operated_trips or {}
    rows = []
    for company, line in sorted(set(planned_by_line) | set(non_operated_by_line), key=lambda item: (item[0].casefold(), item[1])):
        ...
        rows.append({"empresa": company, "linha": line, "km_programado": planned, "km_nao_realizada": non_operated, "viagens_nao_realizadas": non_operated_trips.get((company, line), 0.0), "eficiencia": efficiency})
...
def align_non_operated_companies(planned_by_line, non_operated_by_line):
    """Usa a grafia da PROGRAMADO quando a empresa for a mesma na prática."""
    planned_index = defaultdict(list)
    for company, line in planned_by_line:
        planned_index[(company_key(company), line)].append((company, line))
    aligned = defaultdict(float)
    for (company, line), km in non_operated_by_line.items():
        matches = planned_index.get((company_key(company), line), [])
        # Só troca o nome quando existe uma correspondência única, evitando
        # juntar linhas diferentes por engano.
        target = matches[0] if len(matches) == 1 else (company, line)
        aligned[target] += km
    return aligned
...
def line_values(items, field="km"):
    ...
    for item in items or []:
        company, line = company_name(item.get("empresa", "")), line_key(item.get("linha"))
        if company and line:
            values[(company, line)] += number(item.get(field, item.get("km", 0)))
    return values


def non_operated_summary(km_by_line, trips_by_line):
    summary = defaultdict(lambda: {"km_nao_realizada": 0.0, "viagens_nao_realizadas": 0.0})
    for (company, _line), value in km_by_line.items():
        summary[company]["km_nao_realizada"] += value
    for (company, _line), value in trips_by_line.items():
        summary[company]["viagens_nao_realizadas"] += value
    return [{"empresa": company, **values} for company, values in sorted(summary.items())]
...
def apply_non_operated(payload, non_operated_source, start_date, end_date):
    ...
    non_operated, non_operated_trips = read_non_operated_km(non_operated_source, start_date, end_date)
    non_operated = align_non_operated_companies(planned, non_operated)
    non_operated_trips = align_non_operated_companies(planned, non_operated_trips)
    payload["line_programmed"] = line_items(planned)
    payload["line_non_operated"] = line_items(non_operated)
    payload["lines"] = line_efficiency(planned, non_operated, non_operated_trips)
    payload["non_operated_summary"] = non_operated_summary(non_operated, non_operated_trips)
...
def build_report_from_file(source, destination: Path, start_date, end_date, non_operated_source=None):
    ...
    try:
        ...
        workbook.close()
    payload = {"companies": companies, "daily": daily, "company_daily": company_daily, "line_programmed": line_items(planned_by_line), "line_non_operated": [], "non_operated_summary": [], "lines": line_efficiency(planned_by_line, {}), "has_non_operated_data": False, "parameters": {"bm1": start_date.isoformat(), "bn1": end_date.isoformat()}}
