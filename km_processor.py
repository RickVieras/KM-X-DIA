...
def build_report_from_bytes(source,destination:Path,start_date,end_date):
    try:workbook=load_workbook(io.BytesIO(source),data_only=True,read_only=False)
    ...
    ws_programado["BM1"]=start_date
    ws_programado["BN1"]=end_date
    # O cálculo usa diretamente as mesmas datas para não depender do recálculo
    # de fórmulas pelo Microsoft Excel no servidor.
...
def build_report(sheet_id,destination:Path,start_date,end_date):return build_report_from_bytes(download_source(sheet_id),destination,start_date,end_date)
