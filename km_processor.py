...
def find_non_operated_sheet(workbook):
    required = {"data", "empresa", "km nominal"}
    for ws in workbook.worksheets:
        header = next(ws.iter_rows(min_row=1, max_row=1, values_only=True), ())
        names = {str(value or "").strip().casefold() for value in header}
        if required.issubset(names) and ({"linha", "linha a"} & names):
            return ws
    raise ValueError("A planilha de KM Não Realizada precisa ter as colunas Data, Empresa, Linha ou Linha A, e KM NOMINAL.")
...
def non_operated_columns(header):
    ...
    return {"data": first("data"), "empresa": first("empresa"), "linha": first("linha a", "linha"), "km nominal": first("km nominal")}
