...
def dates_in_period(ws,start_day,end_day):
    if not 1 <= start_day <= end_day <= 31:
        raise ValueError("O período deve estar entre os dias 1 e 31.")
...
def write_daily_sheet(book,name,rows):
    ws=book.create_sheet(name);ws.append(["Data","Frota","Viagens","KM Operacional","KM Morta","KM Transporta","KM Total"])
...
def build_report_from_bytes(source,destination:Path,start_day=1,end_day=31):
    try:workbook=load_workbook(io.BytesIO(source),data_only=True,read_only=True)
