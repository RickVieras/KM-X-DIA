...
def is_transporta(row):return any("transporta" in str(value or "").casefold() for value in row[:COL_MORTA])
...
def calculate(ws,start_day,end_day):
    ...
    for row in ws.iter_rows(min_row=3,values_only=True):
        ...
        for column,current in dates:
            ...
            entry["km_morta"]+=dead;entry["km_transporta"]+=op if transporta else 0;entry["km_total"]+=op+dead
