...
def is_transporta(row):
    return any("transporta" in str(value or "").casefold() for value in row[:COL_MORTA])
...
def calculate(ws,start_day,end_day):
    ...
    result=defaultdict(lambda:defaultdict(lambda:{"frota":0.0,"viagens":0.0,"km_operacional":0.0,"km_morta":0.0,"km_transporta":0.0,"km_total":0.0}))
    ...
    for row in ws.iter_rows(min_row=3,values_only=True):
        ...
        transporta=is_transporta(row)
        if not company and not transporta: continue
        if not company: company="TRANSPORTA"
        ...
        for column,current in dates:
            ...
            op=operational if transporta else trips*operational
            ...
            entry["km_morta"]+=dead
            entry["km_transporta"]+=op if transporta else 0
    ...
    companies=[]
    global_daily=defaultdict(lambda:{"frota":0.0,"viagens":0.0,"km_operacional":0.0,"km_morta":0.0,"km_transporta":0.0,"km_total":0.0})
    company_daily={}
    for company,rows in sorted(result.items()):
        total={"empresa":company,"frota":0.0,"viagens":0.0,"km_operacional":0.0,"km_morta":0.0,"km_transporta":0.0,"km_total":0.0}
        out=[]
        for key,values in sorted(rows.items()):
            out.append({"data":key,**values,"tipos":sorted(types[company][key])})
            for field in ("frota","viagens","km_operacional","km_morta","km_transporta","km_total"):
                total[field]+=values[field]
                global_daily[key][field]+=values[field]
        companies.append(total);company_daily[company]=out
    daily=[{"data":key,**values} for key,values in sorted(global_daily.items())]
