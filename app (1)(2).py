...
def update():
    ...
    try:
        ...
        if upload and upload.filename:
            ...
            if non_operated_upload and non_operated_upload.filename:
                non_operated_extension = Path(non_operated_upload.filename).suffix.casefold()
                if non_operated_extension not in ALLOWED_EXTENSIONS:
                    return jsonify({"error": "Envie a planilha de KM NÃ£o Realizada no formato .xlsx ou .xlsm."}), 400
                with NamedTemporaryFile("wb", suffix=non_operated_extension, dir=OUTPUT_DIR, delete=False) as handle:
                    temporary_non_operated = Path(handle.name)
                    non_operated_upload.save(handle)
                if not temporary_non_operated.stat().st_size:
                    return jsonify({"error": "O arquivo de KM NÃ£o Realizada estÃ¡ vazio."}), 400
            payload = build_report_from_file(temporary_upload, temporary_report, start, end, temporary_non_operated)
        ...
        if temporary_non_operated:
            temporary_non_operated.unlink(missing_ok=True)
