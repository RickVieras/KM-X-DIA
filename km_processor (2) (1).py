...
def line_efficiency(planned_by_line, non_operated_by_line):
    ...
    for company, line in sorted(set(planned_by_line) | set(non_operated_by_line), key=lambda item: (item[0].casefold(), item[1])):
        planned = planned_by_line[(company, line)]
        non_operated = non_operated_by_line[(company, line)]
...
def build_report_from_file(source, destination: Path, start_date, end_date, non_operated_source=None):
    ...
    try:
        ...
        workbook.close()
    non_operated_by_line = read_non_operated_km(non_operated_source, start_date, end_date) if non_operated_source else {}
    lines = line_efficiency(planned_by_line, non_operated_by_line)
    output = Workbook()
    ...
    write_efficiency_sheet(output, lines)
    used = {"total por empresa", "total por dia", "eficiência por linha"}
    company_totals = {item["empresa"]: item for item in companies}
    for company, rows in company_daily.items():
        write_daily_sheet(output, safe_name(company, used), rows, company_totals[company]["km_transporta"])
    destination.parent.mkdir(parents=True, exist_ok=True)
    output.save(destination)
    return {"companies": companies, "daily": daily, "company_daily": company_daily, "lines": lines, "has_non_operated_data": bool(non_operated_source), "parameters": {"bm1": start_date.isoformat(), "bn1": end_date.isoformat()}}
