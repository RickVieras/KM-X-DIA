...
def dates_in_period(ws,start_date,end_date):
    if not start_date or not end_date or start_date > end_date:
        raise ValueError("Informe um período válido para BM1 e BN1.")
    ...
    for column in range(CALENDARIO_INICIO,CALENDARIO_FIM+1):
        ...
        if current and start_date<=current<=end_date:dates.append((column,current))
    if not dates:raise ValueError("Não foram encontradas datas no calendário AE:BI.")
    return dates

def calculate(ws,start_date,end_date):
    ...
    # Nenhuma coluna depois de BI participa do painel. Limitar a leitura evita
    # trazer fórmulas, formatações e dados auxiliares desnecessários.
    for row in ws.iter_rows(min_row=3,max_col=CALENDARIO_FIM,values_only=True):
        ...
        if transporta:transporta_por_empresa[company]+=number(row[COL_TRANSPORTA-1] if len(row)>=COL_TRANSPORTA else 0)
        for column,current in dates:
            ...
            entry["frota"]+=fleet;entry["viagens"]+=trips;entry["km_operacional"]+=op;entry["km_morta"]+=dead;entry["km_total"]+=op+dead
    ...
    for company,rows in sorted(result.items()):
        ...
        total["km_transporta"]=transporta_por_empresa[company];total["km_total"]+=total["km_transporta"]
...
def write_daily_sheet(book,name,rows,monthly_transporta=None):
    ws=book.create_sheet(name);ws.append(["Data","Frota","Viagens","KM Operacional","KM Morta","KM Total"])
    for item in rows:ws.append([item["data"],item["frota"],item["viagens"],item["km_operacional"],item["km_morta"],item["km_total"]])
    if monthly_transporta is not None:
        ...
        for cell in ws[ws.max_row-1]+ws[ws.max_row]:cell.font=__import__("openpyxl").styles.Font(bold=True)
    ws.freeze_panes="A2";ws.auto_filter.ref=ws.dimensions
    for cell in ws[1]:cell.font=__import__("openpyxl").styles.Font(bold=True)
    for row in ws.iter_rows(min_row=2,min_col=1,max_col=1):row[0].number_format="dd/mm/yyyy"
    for column in "ABCDEF":ws.column_dimensions[column].width=22 if column=="A" else 18
    return ws

def build_report_from_file(source,destination:Path,start_date,end_date):
    """Gera o relatório sem carregar a planilha inteira na memória.

    ``read_only`` é essencial no Render: a base pode conter outras abas e
    objetos do Excel que não participam do painel. O cálculo percorre apenas
    os valores da aba PROGRAMADO.
    """
    try:workbook=load_workbook(source,data_only=True,read_only=True,keep_links=False)
    except Exception as error:raise ValueError("O arquivo não é uma planilha Excel válida.") from error
    if "PROGRAMADO" not in workbook.sheetnames:raise ValueError("A planilha precisa ter a aba PROGRAMADO.")
    ws_programado=workbook["PROGRAMADO"]
    # O cálculo usa as datas recebidas diretamente; não é necessário editar
    # BM1/BN1 nem pedir recálculo de fórmulas ao Excel no servidor.
    companies,daily,company_daily=calculate(ws_programado,start_date,end_date)
    ...
    for item in companies:summary.append([item["empresa"],item["frota"],item["viagens"],item["km_operacional"],item["km_morta"],item["km_total"]])
    write_daily_sheet(output,"TOTAL POR DIA",daily);used={"total por empresa","total por dia"}
    company_totals={item["empresa"]:item for item in companies}
    for company,rows in company_daily.items():write_daily_sheet(output,safe_name(company,used),rows,company_totals[company]["km_transporta"])
    destination.parent.mkdir(parents=True,exist_ok=True);output.save(destination)
    workbook.close()
    return {"companies":companies,"daily":daily,"company_daily":company_daily,"parameters":{"bm1":start_date.isoformat(),"bn1":end_date.isoformat()}}

def build_report_from_bytes(source,destination:Path,start_date,end_date):
    """Mantido para compatibilidade com chamadas que já enviam bytes."""
